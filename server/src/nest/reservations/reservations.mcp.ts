...
export class ReservationsMcp {
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      type: z
        .enum(['hotel', 'restaurant', 'event', 'tour', 'activity', 'parking', 'other'])
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      metadata: z
        ...
        .optional()
        .describe(
          'Type-specific metadata: flights → { airline, flight_number, departure_airport, arrival_airport }; trains → { train_number, platform, seat }. Replaces the stored metadata wholesale. Values are plain strings, so per-segment detail belongs in legs[], not here.',
        ),
      endpoints: endpointSchema,
      legs: legsSchema,
      needs_review: z.boolean().optional(),
