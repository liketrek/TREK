...
/**
 * Baseline schema — every table `db/schema.ts::createTables()` created on boot,
 * before the numbered steps in `db/migrations.ts` ran.
 *
 * This is not a frozen "v0": `createTables()` was edited as features landed, so it
 * is a snapshot of the schema as it stood when the ORM migration took place. The
 * `IF NOT EXISTS` guards are kept verbatim, and the numbered steps that follow
 * replay on top of it — each one guarded so a column this file already declares
 * is not added twice.
 *
 * The trailing `schema_version` table is the legacy runner's own bookkeeping,
 * which it created before running step 1. It is left empty here: the migration
 * state now lives in the migrator's table, and nothing in the app reads this
 * one. It is kept because step 129 rebuilds it and it is still an entity.
 */
export class Migration20200101000000_baseline_schema extends Migration {
  override name = 'Migration20200101000000_baseline_schema';
  ...
  override up(): void {
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS idx_prt_user ON password_reset_tokens(user_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_prt_hash ON password_reset_tokens(token_hash)`);
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS idx_collab_notes_trip ON collab_notes(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_collab_polls_trip ON collab_polls(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_collab_messages_trip ON collab_messages(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_places_trip_id ON places(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_places_category_id ON places(category_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_days_trip_id ON days(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_day_assignments_day_id ON day_assignments(day_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_day_assignments_place_id ON day_assignments(place_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_place_tags_place_id ON place_tags(place_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_place_tags_tag_id ON place_tags(tag_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_trip_members_trip_id ON trip_members(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_trip_members_user_id ON trip_members(user_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_packing_items_trip_id ON packing_items(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_budget_items_trip_id ON budget_items(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_reservations_trip_id ON reservations(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_trip_files_trip_id ON trip_files(trip_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_day_notes_day_id ON day_notes(day_id)`);

    this.addSql(`CREATE INDEX IF NOT EXISTS idx_photos_trip_id ON photos(trip_id)`);
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS idx_ncp_user ON notification_channel_preferences(user_id)`);
    ...
    this.addSql(`CREATE TABLE IF NOT EXISTS schema_version (version INTEGER NOT NULL)`);
  }
}
