...
function build(opts: { canEdit?: boolean } = {}) {
  ...
  const assignments = {
    ...
    getAssignmentForTrip: vi.fn((id: number, tripId: number) =>
      id === 30 && tripId === 1 ? { id: 30, day_id: 3 } : undefined,
    ),
...
describe('DaysRpc', () => {
  it('DAYS-RPC-001 writes need the grant, the edit right and a bound user', async () => {
    ...
    expect(((await host.dispatch(req('days.create', { tripId: 2, input: {} }), 42)) as RpcError).error.code).toBe(
      'RESOURCE_FORBIDDEN',
    );
  ...
  it('DAYS-RPC-002 a day on another trip is refused for update and delete alike', async () => {
    ...
    const host = f.host('db:write:days');
    expect(
      ((await host.dispatch(req('days.update', { tripId: 1, dayId: 404, input: {} }), 42)) as RpcError).error.message,
    ).toBe('no day 404 on trip 1');
    expect(((await host.dispatch(req('days.delete', { tripId: 1, dayId: 404 }), 42)) as RpcError).error.message).toBe(
      'no day 404 on trip 1',
    );
...
describe('ItineraryRpc', () => {
  it('ITIN-RPC-001 assign links a day and a place that both belong to the trip', async () => {
    const f = build();
    const res = await f
      .host('db:write:itinerary')
      .dispatch(req('itinerary.assign', { tripId: 1, dayId: 3, placeId: 7 }), 42);
  ...
  it('ITIN-RPC-002 a day or place from another trip is refused, so nothing cross-links', async () => {
    ...
    expect(
      ((await host.dispatch(req('itinerary.assign', { tripId: 1, dayId: 404, placeId: 7 }), 42)) as RpcError).error
        .message,
    ).toBe('no day 404 on trip 1');
    expect(
      ((await host.dispatch(req('itinerary.assign', { tripId: 1, dayId: 3, placeId: 404 }), 42)) as RpcError).error
        .message,
  ...
  it('ITIN-RPC-003 it runs under day_edit, not an itinerary permission of its own', async () => {
    ...
    const res = (await f
      .host('db:write:itinerary')
      .dispatch(req('itinerary.assign', { tripId: 1, dayId: 3, placeId: 7 }), 42)) as RpcError;
  ...
  it('ITIN-RPC-004 notes are optional and pass through as null when absent', async () => {
    ...
    await f
      .host('db:write:itinerary')
      .dispatch(req('itinerary.assign', { tripId: 1, dayId: 3, placeId: 7, notes: 'lunch' }), 42);
  ...
  it('ITIN-RPC-007 an assignment on another trip is refused and nothing is deleted', async () => {
    ...
    const res = (await f
      .host('db:write:itinerary')
      .dispatch(req('itinerary.unassign', { tripId: 1, assignmentId: 404 }), 42)) as RpcError;
