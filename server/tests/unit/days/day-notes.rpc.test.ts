...
describe('DayNotesRpc through the router', () => {
  ...
  it('DAYNOTES-RPC-002 create needs db:write:daynotes + day_edit, membership-checked, text required', async () => {
    ...
    expect(
      (await host.dispatch(req('daynotes.create', { tripId: 1, dayId: 5, input: { text: 'Pack sunscreen' } }), 42)).ok,
    ).toBe(true);
  ...
  it('DAYNOTES-RPC-004 the read grant does not unlock a write', async () => {
    ...
    const res = (await host.dispatch(
      req('daynotes.create', { tripId: 1, dayId: 5, input: { text: 'x' } }),
      42,
    )) as RpcError;
  ...
  it('DAYNOTES-RPC-005 update broadcasts and returns the updated note', async () => {
    const host = f.host('db:write:daynotes');
    expect(
      (await host.dispatch(req('daynotes.update', { tripId: 1, dayId: 5, noteId: 50, input: { text: 'new' } }), 42)).ok,
    ).toBe(true);
    expect(f.realtime.broadcast).toHaveBeenCalledWith(
      1,
      'dayNote:updated',
      expect.objectContaining({ dayId: 5 }),
      undefined,
    );
  });
  ...
  it('DAYNOTES-RPC-006 a note on another day is refused, naming it', async () => {
    const host = f.host('db:write:daynotes');
    const res = (await host.dispatch(
      req('daynotes.update', { tripId: 1, dayId: 5, noteId: 404, input: {} }),
      42,
    )) as RpcError;
  ...
  it('DAYNOTES-RPC-008 a day outside the trip is refused, naming it', async () => {
    const host = f.host('db:write:daynotes');
    const res = (await host.dispatch(
      req('daynotes.create', { tripId: 1, dayId: 99, input: { text: 'x' } }),
      42,
    )) as RpcError;
