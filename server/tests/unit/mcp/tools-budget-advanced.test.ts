...
describe('Settlement tools', () => {
  ...
  it('creates a foreign-currency settlement from a server-owned global snapshot', async () => {
    const { user, other, trip } = tripWithTwo();
    testDb
      .prepare(
        `
      ...
      VALUES ('EUR', '{"EUR":1,"USD":1.25}', 'snapshot:2',
              '2026-08-07', ?)
    `,
      )
  ...
  it('records a supplied rate as explicit when updating a settlement', async () => {
    const { user, other, trip } = tripWithTwo();
