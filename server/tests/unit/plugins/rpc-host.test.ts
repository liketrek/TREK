...
describe('PluginRpcHost — capability enforcement', () => {
  ...
  it('daynotes.create needs db:write:daynotes + day_edit, membership-checked, text required', async () => {
    ...
    const good = await host.dispatch(req('daynotes.create', { tripId: 1, dayId: 5, input: { text: 'Pack sunscreen', color: 'amber' } }), 42);
    expect(ok(good)).toBe(true);
    expect(deps.createDayNote).toHaveBeenCalledWith(1, 5, expect.objectContaining({ text: 'Pack sunscreen', color: 'amber' }));
