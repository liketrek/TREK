...
function build() {
  const categories = { list: vi.fn(() => [{ id: 1, name: 'Food' }]) } as unknown as CategoriesService &
    Record<string, ReturnType<typeof vi.fn>>;
  const weather = { get: vi.fn(async () => ({ temp: 20 })) } as unknown as WeatherService &
    Record<string, ReturnType<typeof vi.fn>>;
  const rates = {
    getRates: vi.fn(async (base: string) => ({ [base]: 1, USD: 1.08 })),
  } as unknown as ExchangeRatesService & Record<string, ReturnType<typeof vi.fn>>;
...
describe('tenant-free plugin methods', () => {
  ...
  it('TENANTFREE-003 weather coordinates are coerced, and the language stays pinned to en', async () => {
    ...
    await f
      .host('weather:read')
      .dispatch(req('weather.get', { lat: '48.1', lng: '11.5', date: '2027-01-01' }), undefined);
  ...
  it('TENANTFREE-004 a non-numeric coordinate is BAD_PARAMS', async () => {
    const res = (await build()
      .host('weather:read')
      .dispatch(req('weather.get', { lat: 'north', lng: 11 }), undefined)) as RpcError;
  ...
  it('TENANTFREE-005 a non-string base is BAD_PARAMS', async () => {
    const res = (await build()
      .host('rates:read')
      .dispatch(req('rates.get', { base: 42 }), undefined)) as RpcError;
    expect(res.error.code).toBe('BAD_PARAMS');
  });
