...
describe('optional fields fall back rather than reaching the service as undefined', () => {
  ...
  it('FALLBACK-002 a day note without a text falls back to an empty string', async () => {
    ...
    const host = new PluginRpcHost(
      ...
      ALL,
      makeDeps(),
      createTestPluginRegistry([new DayNotesRpc(notes, realtime(), guards)]),
    );
  ...
  it('FALLBACK-003 a file link with no targets stores three nulls', async () => {
    ...
    const host = new PluginRpcHost(
      ...
      ALL,
      makeDeps(),
      createTestPluginRegistry([new FilesRpc(files, realtime(), db, guards, {} as never)]),
    );
  ...
  it('FALLBACK-004 a file update tells an omitted link from one cleared with null', async () => {
    ...
    await host.dispatch(
      req('files.update', { tripId: 1, fileId: 2, input: { reservation_id: null, place_id: 7 } }),
      42,
    );
    expect(updateFile).toHaveBeenLastCalledWith(
      2,
      expect.anything(),
      expect.objectContaining({ reservation_id: null, place_id: '7' }),
    );
  });
...
describe('a service that says no becomes a refusal, not a crash', () => {
  it('FALLBACK-006 a place that will not delete is refused', async () => {
    ...
    const host = new PluginRpcHost(
      ...
      ALL,
      makeDeps(),
      createTestPluginRegistry([new PlacesRpc(places, journey, realtime(), guards)]),
    );
    expect(err(await host.dispatch(req('places.delete', { tripId: 1, placeId: 7 }), 42)).message).toBe(
      'no place 7 on trip 1',
    );
  });
  ...
  it('FALLBACK-007 a bag that will not update or delete is refused', async () => {
    const { guards } = guardsFor();
    const packing = {
      updateBag: vi.fn(() => null),
      deleteBag: vi.fn(() => false),
      setBagMembers: vi.fn(() => null),
    } as never;
    const host = new PluginRpcHost(
      'p',
      ALL,
    ...
    expect(err(await host.dispatch(req('packing.updateBag', { tripId: 1, bagId: 80, input: {} }), 42)).message).toBe(
      'no packing bag 80 on trip 1',
    );
    expect(err(await host.dispatch(req('packing.deleteBag', { tripId: 1, bagId: 80 }), 42)).message).toBe(
      'no packing bag 80 on trip 1',
    );
    expect(
      err(await host.dispatch(req('packing.setBagMembers', { tripId: 1, bagId: 80, userIds: [] }), 42)).message,
    ).toBe('no packing bag 80 on trip 1');
  });
  ...
  it('FALLBACK-008 an accommodation whose refs do not validate is refused with the validator message', async () => {
    ...
    const host = new PluginRpcHost(
      ...
      ALL,
      makeDeps(),
      createTestPluginRegistry([new AccommodationsRpc(accommodations, realtime(), guards)]),
    );
    expect(
      err(
        await host.dispatch(
          req('accommodations.update', { tripId: 1, accommodationId: 11, input: { start_day_id: 9 } }),
          42,
        ),
      ).message,
...
describe('service errors are translated into the RPC taxonomy', () => {
  ...
  it('FALLBACK-009 a trip update maps ValidationError and NotFoundError separately', async () => {
    ...
    expect(err(await invalid.dispatch(req('trips.update', { tripId: 1, input: { title: 'x' } }), 42))).toMatchObject({
      code: 'BAD_PARAMS',
      message: 'end before start',
    });
    const missing = tripsHost(new NotFoundError('trip is gone'));
    expect(err(await missing.dispatch(req('trips.update', { tripId: 1, input: { title: 'x' } }), 42))).toMatchObject({
      code: 'RESOURCE_FORBIDDEN',
      message: 'trip is gone',
    });
  });

  it('FALLBACK-010 an unknown trip failure keeps its own taxonomy', async () => {
    const boom = tripsHost(new Error('disk on fire'));
    expect(err(await boom.dispatch(req('trips.update', { tripId: 1, input: { title: 'x' } }), 42))).toMatchObject({
      code: 'HOST_ERROR',
  ...
  it('FALLBACK-011 trips.create maps a ValidationError too, and passes anything else on', async () => {
    ...
    expect(err(await invalid.dispatch(req('trips.create', { input: { title: 'x' } }), 42))).toMatchObject({
      code: 'BAD_PARAMS',
      message: 'title too short',
    });
    const boom = tripsHost(undefined, new Error('db locked'));
    expect(err(await boom.dispatch(req('trips.create', { input: { title: 'x' } }), 42))).toMatchObject({
      code: 'HOST_ERROR',
    });
  });
  ...
  it('FALLBACK-012 a reservation delete without a type still notifies with an empty one', async () => {
    ...
    const reservations = {
      remove: vi.fn(() => ({
        deleted: { title: 'Hotel', type: null, accommodation_id: null },
    ...
    const host = new PluginRpcHost(
      ...
      ALL,
      makeDeps(),
      createTestPluginRegistry([new ReservationsRpc(reservations, realtime(), guards)]),
    );
  ...
  it('FALLBACK-013 a reservation update falls back to the stored type when none is sent', async () => {
    ...
    await host.dispatch(
      req('reservations.update', { tripId: 1, reservationId: 5, input: { title: 'Renamed', type: 'lodging' } }),
      42,
    );
...
describe('schemaMessage is the one copy of a fallback that used to exist twenty times', () => {
  it('FALLBACK-014 it reports the first issue when there is one', () => {
    expect(schemaMessage({ issues: [{ message: 'name is required' }, { message: 'second' }] })).toBe(
      'name is required',
    );
  });
...
describe('every schema-validated method rejects a payload its schema refuses', () => {
  function everything() {
    ...
    const anything = new Proxy(
      {},
      { get: () => vi.fn(() => ({ id: 1, trip: { id: 1 }, updatedTrip: { id: 1 }, reservation: { id: 1 } })) },
    ) as never;
  ...
  it('FALLBACK-016 a reservation with malformed endpoints is rejected on update too', async () => {
    const res = await everything().dispatch(
      req('reservations.update', {
        tripId: 1,
        reservationId: 5,
        input: { title: 'x', type: 'lodging', endpoints: [{ junk: 1 }] },
      }),
      42,
    );
    expect(err(res).code).toBe('BAD_PARAMS');
