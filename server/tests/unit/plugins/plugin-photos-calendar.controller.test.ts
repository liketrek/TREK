...
function runtime(providers: string[], invoke: (id: string, fn: string, args: unknown[]) => unknown) {
  return {
    ...
    searchPhotos: vi.fn(async (id: string, query: string, page: number, limit: number) =>
      invoke(id, 'search', [query, { page, limit }]),
    ),
    ...
    calendarEvents: vi.fn(async (id: string, userId: number, start: string, end: string) =>
      invoke(id, 'getEvents', [userId, start, end]),
    ),
