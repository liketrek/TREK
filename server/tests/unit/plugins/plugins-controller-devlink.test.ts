...
const ctrl = (runtime: any) =>
  new PluginsController({} as any, runtime, {} as any, { isManaged: () => false } as unknown as RuntimeEnvService);
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const status = async (p: Promise<any>): Promise<number> =>
  p.then(
    () => -1,
    (e) => (e instanceof HttpException ? e.getStatus() : 0),
  );
...
describe('PluginsController dev-link endpoints', () => {
  ...
  it('POST /:id/reload maps a re-consent error to 409 and otherwise returns the live status', async () => {
    ...
    const consent = {
      reload: vi.fn(async () => {
        throw new PluginConsentRequired('needs consent', ['db:read:trips'], []);
      }),
      isActive: () => false,
    };
