...
describe('CollabController (parity with the legacy /api/trips/:tripId/collab route)', () => {
  describe('notes', () => {
    ...
    it('POST creates + broadcasts + notifies (empty title now 400s in the Zod pipe)', () => {
      ...
      expect(new CollabController(s, storageStub).createNote(user, '5', { title: 'T', content: 'c' }, 'sock')).toEqual({
        note: { id: 9 },
      });
    ...
    it('PUT 404 when missing, else updates + broadcasts', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ updateNote: vi.fn().mockReturnValue(null) } as Partial<CollabService>),
            storageStub,
          ).updateNote(user, '5', '9', {}),
        ),
      ...
      expect(new CollabController(s, storageStub).updateNote(user, '5', '9', { title: 'b' }, 'sock')).toEqual({
        note: { id: 9 },
      });
  ...
  describe('note files', () => {
    ...
    it('403 without file_upload, 400 without file, 404 unknown note, else commits + returns result', async () => {
      expect(
        await thrownAsync(() =>
          new CollabController(svc({ canUploadFiles: vi.fn().mockReturnValue(false) }), storageStub).addNoteFile(
            user,
            '5',
            '9',
            file,
      ...
      expect(
        await thrownAsync(() =>
          new CollabController(
            svc({ addNoteFile: vi.fn().mockReturnValue(null) } as Partial<CollabService>),
            storageStub,
      ...
      const broadcast = vi.fn();
      const s = svc({
        addNoteFile: vi.fn().mockReturnValue({ file: { id: 3 } }),
        getFormattedNoteById: vi.fn().mockReturnValue({ id: 9 }),
        broadcast,
    ...
    it('reclaims the upload on every refusal, spooled or already committed', async () => {
      ...
      expect(
        await thrownAsync(() =>
          new CollabController(svc({ verifyTripAccess: vi.fn().mockReturnValue(null) }), storageStub).addNoteFile(
            user,
            '5',
            '9',
  ...
  describe('polls', () => {
    it('POST creates (missing question / <2 options now 400 in the Zod pipe)', () => {
      ...
      expect(
        new CollabController(s, storageStub).createPoll(user, '5', { question: 'q', options: ['a', 'b'] }),
      ).toEqual({ poll: { id: 7 } });
    });
    ...
    it('vote maps not_found/closed/invalid_index, else broadcasts the poll', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ votePoll: vi.fn().mockReturnValue({ error: 'not_found' }) } as Partial<CollabService>),
            storageStub,
      ...
      expect(
        thrown(() =>
          new CollabController(
            svc({ votePoll: vi.fn().mockReturnValue({ error: 'closed' }) } as Partial<CollabService>),
            storageStub,
          ).votePoll(user, '5', '7', { option_index: 0 }),
        ),
      ...
      expect(
        thrown(() =>
          new CollabController(
            svc({ votePoll: vi.fn().mockReturnValue({ error: 'invalid_index' }) } as Partial<CollabService>),
            storageStub,
      ...
      expect(new CollabController(s, storageStub).votePoll(user, '5', '7', { option_index: 0 }, 'sock')).toEqual({
        poll: { id: 7 },
      });
    ...
    it('close 404 when missing, else broadcasts', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ closePoll: vi.fn().mockReturnValue(null) } as Partial<CollabService>),
            storageStub,
          ).closePoll(user, '5', '7'),
        ),
      ).toEqual({ status: 404, body: { error: 'Poll not found' } });
    ...
    it('delete 404 when missing, else success', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ deletePoll: vi.fn().mockReturnValue(false) } as Partial<CollabService>),
            storageStub,
          ).deletePoll(user, '5', '7'),
        ),
      ).toEqual({ status: 404, body: { error: 'Poll not found' } });
  ...
  describe('messages', () => {
    it('POST 400 whitespace-only, 400 reply_not_found, else creates + notifies (length checks now in the Zod pipe)', () => {
      ...
      const notifyCollab = vi.fn();
      const s = svc({
        createMessage: vi.fn().mockReturnValue({ message: { id: 3 } }),
        broadcast,
        notifyCollab,
      } as Partial<CollabService>);
    ...
    it('react 404 unknown, else broadcasts reactions (empty emoji now 400s in the Zod pipe)', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ reactMessage: vi.fn().mockReturnValue({ found: false, reactions: [] }) } as Partial<CollabService>),
            storageStub,
      ...
      const broadcast = vi.fn();
      const s = svc({
        reactMessage: vi.fn().mockReturnValue({ found: true, reactions: [{ emoji: '👍', count: 1 }] }),
        broadcast,
      } as Partial<CollabService>);
      expect(new CollabController(s, storageStub).react(user, '5', '3', { emoji: '👍' }, 'sock')).toEqual({
    ...
    it('delete maps not_found (404) / not_owner (403), else success with username', () => {
      expect(
        thrown(() =>
          new CollabController(
            svc({ deleteMessage: vi.fn().mockReturnValue({ error: 'not_found' }) } as Partial<CollabService>),
            storageStub,
      ...
      expect(
        thrown(() =>
          new CollabController(
            svc({ deleteMessage: vi.fn().mockReturnValue({ error: 'not_owner' }) } as Partial<CollabService>),
            storageStub,
          ).deleteMessage(user, '5', '3'),
        ),
      ...
      const broadcast = vi.fn();
      const s = svc({
        deleteMessage: vi.fn().mockReturnValue({ username: 'bob' }),
        broadcast,
      } as Partial<CollabService>);
