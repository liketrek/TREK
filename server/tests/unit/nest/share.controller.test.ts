...
describe('TripShareController', () => {
  ...
  it('GET 404 without access, 403 without share_manage, returns info or a null token', () => {
    expect(
      thrown(() =>
        new TripShareController(svc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).get(user, '5'),
      ),
    ).toEqual({ status: 404, body: { error: 'Trip not found' } });
    ...
    expect(
      thrown(() =>
        new TripShareController(svc({ canManage: vi.fn().mockReturnValue(false), get } as Partial<ShareService>)).get(
          user,
          '5',
        ),
      ),
    ...
    expect(
      new TripShareController(svc({ get: vi.fn().mockReturnValue({ token: 't' }) } as Partial<ShareService>)).get(
        user,
        '5',
      ),
    ).toEqual({ token: 't' });
    expect(
      new TripShareController(svc({ get: vi.fn().mockReturnValue(null) } as Partial<ShareService>)).get(user, '5'),
    ).toEqual({ token: null });
  });
    ...
    expect(
      thrown(() =>
        new SharedController(
          svc({ getSharedTripData: vi.fn().mockReturnValue(null) } as Partial<ShareService>),
          storageStub,
        ).read('bad'),
      ),
    ).toEqual({ status: 404, body: { error: 'Invalid or expired link' } });
    expect(
      new SharedController(
        svc({ getSharedTripData: vi.fn().mockReturnValue({ trip: { id: 9 } }) } as Partial<ShareService>),
        storageStub,
      ).read('tok'),
    ).toEqual({ trip: { id: 9 } });
  });
  ...
  describe('place-photo proxy', () => {
    ...
    function photoRes() {
      ...
      const r = Object.assign(sink, {
        ...
        status: vi.fn(function (this: unknown, c: number) {
          (r as { statusCode: number }).statusCode = c;
          return r;
        }),
        json: vi.fn(),
        set: vi.fn(),
    ...
    function controller(key: string | null) {
      return new SharedController(
        svc({ getSharedPlacePhotoKey: vi.fn().mockReturnValue(key) } as Partial<ShareService>),
        storageStub,
      );
    }
