 ...
 */
/**
 * The index switch reads fail-OPEN, like places_enrich_enabled and unlike the
 * three Google flags: nothing backfills a row on upgrade, so an absent row has
 * to mean on, or every existing install would silently lose the index.
 *
 * It has to keep agreeing with MapsService.trekPlacesEnabled(), which reads the
 * same key itself. If the two diverge, the admin panel shows "off" while every
 * search and keystroke keeps leaving the instance.
 */
describe('AddonsService trek places flag', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('ADDONS-SVC-094 an unset flag reads as ON, and only the literal "false" turns it off', () => {
    dbMock._stmt.get.mockReturnValueOnce(undefined);
    expect(svc().getTrekPlaces()).toEqual({ enabled: true });
    expect(dbMock.prepare).toHaveBeenLastCalledWith(
      "SELECT value FROM app_settings WHERE key = 'trek_places_enabled'",
    );

    dbMock._stmt.get.mockReturnValueOnce({ value: 'false' });
    expect(svc().getTrekPlaces()).toEqual({ enabled: false });

    // Anything else is on. A garbled row must not silently disable the default
    // search path for a whole install.
    for (const value of ['true', 'FALSE', '0', '']) {
      dbMock._stmt.get.mockReturnValueOnce({ value });
      expect(svc().getTrekPlaces(), value).toEqual({ enabled: true });
    }
  });

  it('ADDONS-SVC-095 the setter round-trips through the getter under its own key', () => {
    // Keyed store rather than an echo assertion: the write has to produce the
    // exact string the read compares against, so a setter persisting '0' fails
    // here instead of silently reading back ON in production.
    //
    // The reader inlines its key in the SQL and binds nothing, the way the
    // other fail-open flag does, so the statement text is what identifies the
    // row here — not the bound parameter the writer uses.
    ...
    dbMock._stmt.get.mockImplementation((key?: string) => {
      const wanted = key ?? (lastSql.match(/key = '([^']+)'/)?.[1] ?? '');
      const value = stored.get(wanted);
      return value === undefined ? undefined : { value };
    });

    expect(svc().updateTrekPlaces(false)).toEqual({ enabled: false });
    expect(dbMock._stmt.run).toHaveBeenLastCalledWith('trek_places_enabled', 'false');
    expect(svc().getTrekPlaces()).toEqual({ enabled: false });

    expect(svc().updateTrekPlaces(true)).toEqual({ enabled: true });
    expect(dbMock._stmt.run).toHaveBeenLastCalledWith('trek_places_enabled', 'true');
    expect(svc().getTrekPlaces()).toEqual({ enabled: true });
    expect([...stored.keys()]).toEqual(['trek_places_enabled']);
  });

  it('ADDONS-SVC-096 answers the same as MapsService.trekPlacesEnabled() for every stored value', () => {
    // The admin panel and the six call paths that gate on this must never
    // disagree: showing "off" while queries keep leaving is worse than either
    // default on its own.
    const maps = new MapsService(new DatabaseService(dbConn), {} as never);
    ...
    for (const [row, expected] of rows) {
      ...
      expect(svc().getTrekPlaces()).toEqual({ enabled: expected });
      expect(maps.trekPlacesEnabled()).toBe(expected);
    }
  });
});
