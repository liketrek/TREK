...
describe('JourneyController', () => {
  it('GET / lists; POST / 400 without title, else creates', () => {
    expect(
      ctl(svc({ listJourneys: vi.fn().mockReturnValue([{ id: 1 }]) } as Partial<JourneyService>)).list(user),
    ).toEqual({ journeys: [{ id: 1 }] });
    expect(thrown(() => ctl(svc()).create(user, { title: '   ' }))).toEqual({
      status: 400,
      body: { error: 'Title is required' },
    });
    const createJourney = vi.fn().mockReturnValue({ id: 9 });
    expect(
      ctl(svc({ createJourney } as Partial<JourneyService>)).create(user, { title: ' Trip ', trip_ids: [1, '2'] }),
    ).toEqual({ id: 9 });
  ...
  it('GET /suggestions + /available-trips', () => {
    expect(
      ctl(svc({ getSuggestions: vi.fn().mockReturnValue([{ id: 1 }]) } as Partial<JourneyService>)).suggestions(user),
    ).toEqual({ trips: [{ id: 1 }] });
    expect(
      ctl(svc({ listUserTrips: vi.fn().mockReturnValue([{ id: 2 }]) } as Partial<JourneyService>)).availableTrips(user),
    ).toEqual({ trips: [{ id: 2 }] });
  });
  ...
  it('PATCH/DELETE entries map 404', () => {
    expect(
      thrown(() =>
        ctl(svc({ updateEntry: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).updateEntry(user, '3', {}),
      ),
    ).toEqual({ status: 404, body: { error: 'Entry not found' } });
    expect(
      ctl(svc({ updateEntry: vi.fn().mockReturnValue({ id: 3 }) } as Partial<JourneyService>)).updateEntry(user, '3', {
        title: 'x',
      }),
    ).toEqual({ id: 3 });
    expect(
      thrown(() =>
        ctl(svc({ deleteEntry: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).deleteEntry(user, '3'),
      ),
    ).toEqual({ status: 404, body: { error: 'Entry not found' } });
    expect(
      ctl(svc({ deleteEntry: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).deleteEntry(user, '3'),
  ...
  it('provider-photos: batch, single 400/403, success', () => {
    ...
    expect(ctl(batch).providerPhotos(user, '3', { provider: 'immich', asset_ids: ['a', 'b'] })).toEqual({
      photos: [{ id: 1 }, { id: 1 }],
      added: 2,
    });
    expect(thrown(() => ctl(svc()).providerPhotos(user, '3', { provider: 'immich' }))).toEqual({
      status: 400,
      body: { error: 'provider and asset_id required' },
    });
    expect(
      thrown(() =>
        ctl(svc({ addProviderPhoto: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).providerPhotos(
          user,
          '3',
          { provider: 'immich', asset_id: 'a' },
        ),
      ),
  ...
  it('link-photo: 400 without id (accepts legacy photo_id), 403, success', () => {
    expect(thrown(() => ctl(svc()).linkPhoto(user, '3', {}))).toEqual({
      status: 400,
    ...
    expect(
      thrown(() =>
        ctl(svc({ linkPhotoToEntry: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).linkPhoto(user, '3', {
          journey_photo_id: 9,
        }),
      ),
  ...
  it('unlink photo (204) maps 404; delete photo 404 then removes the object', async () => {
    expect(
      thrown(() =>
        ctl(svc({ unlinkPhotoFromEntry: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).unlinkPhoto(
          user,
          '3',
          '7',
        ),
    ...
    expect(
      ctl(svc({ unlinkPhotoFromEntry: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).unlinkPhoto(
        user,
        '3',
        '7',
      ),
    ...
    expect(
      await thrownAsync(() =>
        ctl(svc({ deletePhoto: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).deletePhoto(user, '7'),
      ),
  ...
  it('gallery upload 400 no files / 403 not allowed, else commits + returns photos', async () => {
    expect(await thrownAsync(() => ctl(svc()).uploadGalleryPhotos(user, '3', undefined))).toEqual({
      status: 400,
      body: { error: 'No files uploaded' },
    });
    expect(
      await thrownAsync(() =>
        ctl(svc({ uploadGalleryPhotos: vi.fn().mockReturnValue([]) } as Partial<JourneyService>)).uploadGalleryPhotos(
          user,
          '3',
  ...
  it('gallery video: 400 no video, 403 not allowed, else stores the clip + poster (#823)', async () => {
    const files = {
      video: [{ filename: 'v.mp4' } as Express.Multer.File],
      poster: [{ filename: 'p.jpg' } as Express.Multer.File],
    };
    expect(await thrownAsync(() => ctl(svc()).uploadGalleryVideo(user, '3', {}, {}))).toEqual({
      status: 400,
      body: { error: 'No video uploaded' },
    });
    ...
    const withPaths = {
      video: [{ filename: 'v.mp4', path: '/nonexistent/v.mp4' } as Express.Multer.File],
      poster: [{ filename: 'p.jpg', path: '/nonexistent/p.jpg' } as Express.Multer.File],
    };
    expect(
      await thrownAsync(() =>
        ctl(svc({ uploadGalleryPhotos: vi.fn().mockReturnValue([]) } as Partial<JourneyService>)).uploadGalleryVideo(
          user,
          '3',
          withPaths,
          { duration_ms: 'abc' },
    ...
    expect(
      await ctl(svc({ uploadGalleryPhotos: up } as Partial<JourneyService>)).uploadGalleryVideo(user, '3', files, {
        duration_ms: '4200',
      }),
    ).toEqual({ photos: [{ id: 7 }] });
    expect(up).toHaveBeenCalledWith(3, 1, [
      { path: 'journey/v.mp4', thumbnail: 'journey/p.jpg', mediaType: 'video', durationMs: 4200 },
    ]);

    // No poster + no duration → thumbnail undefined, durationMs null.
    const up2 = vi.fn().mockReturnValue([{ id: 8 }]);
    await ctl(svc({ uploadGalleryPhotos: up2 } as Partial<JourneyService>)).uploadGalleryVideo(
      user,
      '3',
      { video: [{ filename: 'v2.mp4' } as Express.Multer.File] },
      {},
    );
    expect(up2).toHaveBeenCalledWith(3, 1, [
      { path: 'journey/v2.mp4', thumbnail: undefined, mediaType: 'video', durationMs: null },
    ]);
  });

  it('provider-photos forwards per-asset media_types for gallery and entries (#823)', () => {
    const add = vi.fn().mockReturnValue({ id: 1 });
    ctl(svc({ addProviderPhotoToGallery: add } as Partial<JourneyService>)).galleryProviderPhotos(user, '9', {
      provider: 'immich',
      asset_ids: ['a', 'b'],
    ...
    ctl(svc({ addProviderPhotoToGallery: addOne } as Partial<JourneyService>)).galleryProviderPhotos(user, '9', {
      provider: 'immich',
      asset_id: 'c',
      media_type: 'video',
    });
    ...
    ctl(svc({ addProviderPhoto: eAdd } as Partial<JourneyService>)).providerPhotos(user, '4', {
      provider: 'immich',
      asset_ids: ['x'],
      media_types: ['video'],
      caption: 'c',
    ...
    ctl(svc({ addProviderPhoto: eOne } as Partial<JourneyService>)).providerPhotos(user, '4', {
      provider: 'immich',
      asset_id: 'y',
      media_type: 'video',
    });
  ...
  it('GET/PATCH/DELETE /:id map 404', () => {
    expect(
      thrown(() =>
        ctl(svc({ getJourneyFull: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).get(user, '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Journey not found' } });
    expect(
      ctl(svc({ getJourneyFull: vi.fn().mockReturnValue({ id: 9 }) } as Partial<JourneyService>)).get(user, '9'),
    ).toEqual({ id: 9 });
    expect(
      thrown(() =>
        ctl(svc({ updateJourney: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).update(user, '9', {}),
      ),
    ).toEqual({ status: 404, body: { error: 'Journey not found' } });
    expect(
      thrown(() =>
        ctl(svc({ deleteJourney: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).remove(user, '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Journey not found' } });
  });

  it('trips: POST 400 without trip_id / 403, DELETE 403', () => {
    expect(thrown(() => ctl(svc()).addTrip(user, '9', {}))).toEqual({
      status: 400,
      body: { error: 'trip_id required' },
    });
    expect(
      thrown(() =>
        ctl(svc({ addTripToJourney: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).addTrip(user, '9', {
          trip_id: 2,
        }),
      ),
    ).toEqual({ status: 403, body: { error: 'Not allowed' } });
    expect(
      ctl(svc({ addTripToJourney: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).addTrip(user, '9', {
        trip_id: 2,
      }),
    ).toEqual({ success: true });
    expect(
      thrown(() =>
        ctl(svc({ removeTripFromJourney: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).removeTrip(
          user,
          '9',
          '2',
        ),
  ...
  it('entries under journey: list 404, create 400/404, reorder 400/403', () => {
    expect(
      thrown(() =>
        ctl(svc({ listEntries: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).listEntries(user, '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Journey not found' } });
    expect(
      ctl(svc({ listEntries: vi.fn().mockReturnValue([{ id: 1 }]) } as Partial<JourneyService>)).listEntries(user, '9'),
    ).toEqual({ entries: [{ id: 1 }] });
    expect(thrown(() => ctl(svc()).createEntry(user, '9', {}))).toEqual({
      status: 400,
      body: { error: 'entry_date is required' },
    });
    expect(
      thrown(() =>
        ctl(svc({ createEntry: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).createEntry(user, '9', {
          entry_date: '2026-01-01',
        }),
      ),
    ).toEqual({ status: 404, body: { error: 'Journey not found' } });
    expect(thrown(() => ctl(svc()).reorderEntries(user, '9', { orderedIds: 'no' }))).toEqual({
      status: 400,
      body: { error: 'orderedIds must be an array of numbers' },
    });
    expect(
      thrown(() =>
        ctl(svc({ reorderEntries: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).reorderEntries(
          user,
          '9',
          { orderedIds: [1, 2] },
        ),
  ...
  it('contributors: add 400/403, update 403, remove 403', () => {
    expect(thrown(() => ctl(svc()).addContributor(user, '9', {}))).toEqual({
      status: 400,
      body: { error: 'user_id required' },
    });
    expect(
      thrown(() =>
        ctl(svc({ addContributor: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).addContributor(
          user,
          '9',
          { user_id: 2 },
        ),
    ...
    expect(
      ctl(svc({ addContributor: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).addContributor(user, '9', {
        user_id: 2,
      }),
    ).toEqual({ success: true });
    expect(
    ...
    expect(
      thrown(() =>
        ctl(svc({ removeContributor: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).removeContributor(
          user,
          '9',
          '2',
        ),
  ...
  it('preferences 403, share-link get/set/delete', () => {
    expect(
      thrown(() =>
        ctl(svc({ updateJourneyPreferences: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).preferences(
          user,
          '9',
          {},
        ),
    ...
    expect(
      ctl(
        svc({
          getJourneyShareLink: vi.fn().mockReturnValue({ allowed: true, link: { token: 'abc' } }),
    ...
    expect(
      ctl(
        svc({ getJourneyShareLink: vi.fn().mockReturnValue({ allowed: true, link: null }) } as Partial<JourneyService>),
      ).getShareLink(user, '9'),
    ).toEqual({ link: null });
    expect(
      thrown(() =>
        ctl(
          svc({ getJourneyShareLink: vi.fn().mockReturnValue({ allowed: false }) } as Partial<JourneyService>),
        ).getShareLink(user, '9'),
      ),
    ).toEqual({ status: 403, body: { error: 'Not allowed' } });
    expect(
    ...
    expect(
      ctl(svc({ createOrUpdateJourneyShareLink: setLink } as Partial<JourneyService>)).setShareLink(user, '9', {
        share_timeline: true,
      }),
    ).toEqual({ token: 'abc' });
    ...
    expect(setLink).toHaveBeenCalledWith(9, 1, {
      share_timeline: true,
      share_gallery: undefined,
      share_map: undefined,
      newest_first: undefined,
    });
    const withOrder = vi.fn().mockReturnValue({ token: 'abc' });
    ctl(svc({ createOrUpdateJourneyShareLink: withOrder } as Partial<JourneyService>)).setShareLink(user, '9', {
      newest_first: true,
    });
    expect(withOrder).toHaveBeenCalledWith(9, 1, {
      share_timeline: undefined,
      share_gallery: undefined,
      share_map: undefined,
      newest_first: true,
    });
    expect(
      thrown(() =>
        ctl(svc({ deleteJourneyShareLink: vi.fn().mockReturnValue(false) } as Partial<JourneyService>)).deleteShareLink(
          user,
          '9',
        ),
      ),
  ...
  it('takes the committed bytes back out when the upload is refused', async () => {
    ...
    await thrownAsync(() =>
      ctl(svc({ addPhoto: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).uploadEntryPhotos(
        user,
        '3',
        [file],
        {},
    ...
    await thrownAsync(() =>
      ctl(svc({ uploadGalleryPhotos: vi.fn().mockReturnValue([]) } as Partial<JourneyService>)).uploadGalleryPhotos(
        user,
        '3',
        [{ filename: 'g.jpg' } as Express.Multer.File],
      ),
    );
    ...
    vi.clearAllMocks();
    await thrownAsync(() =>
      ctl(svc({ updateJourney: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).cover(user, '9', {
        filename: 'c.jpg',
  ...
  it('drops the files of a partly refused entry batch, keeping the accepted ones', async () => {
    ...
    const res = await ctl(svc({ addPhoto, immichAutoUploadEnabled } as Partial<JourneyService>)).uploadEntryPhotos(
      user,
      '3',
      files,
      {},
  ...
  it('entry photo upload mirrors to Immich only when opted in', async () => {
    ...
    const res = await ctl(s).uploadEntryPhotos(
      user,
      '3',
    ...
    await ctl(noOptIn).uploadEntryPhotos(
      user,
      '3',
  ...
  it('entry photo upload: 400 no files, 403 when nothing added, swallows immich errors and empty ids', async () => {
    expect(await thrownAsync(() => ctl(svc()).uploadEntryPhotos(user, '3', undefined, {}))).toEqual({
      status: 400,
      body: { error: 'No files uploaded' },
    });
    expect(
    ...
    expect(
      await ctl(blowsUp).uploadEntryPhotos(
        user,
        '3',
        [{ filename: 'a.jpg', originalname: 'a.jpg' } as Express.Multer.File],
        { caption: 'c' },
    ...
    expect(
      await ctl(noId).uploadEntryPhotos(
        user,
        '3',
        [{ filename: 'a.jpg', originalname: 'a.jpg' } as Express.Multer.File],
        {},
  ...
  it('provider-photos batch passes the passphrase through when present', () => {
    ...
    ctl(svc({ addProviderPhoto } as Partial<JourneyService>)).providerPhotos(user, '3', {
      provider: 'immich',
      asset_ids: ['a'],
      caption: 'cap',
      passphrase: 'secret',
    ...
    expect(
      ctl(svc({ addProviderPhoto: vi.fn().mockReturnValue({ id: 2 }) } as Partial<JourneyService>)).providerPhotos(
        user,
        '3',
        { provider: 'immich', asset_id: 'a' },
      ),
  ...
  it('PATCH photos: 404 then returns the updated photo', () => {
    expect(
      thrown(() =>
        ctl(svc({ updatePhoto: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).updatePhoto(user, '7', {
          caption: 'x',
        }),
      ),
    ).toEqual({ status: 404, body: { error: 'Photo not found' } });
    expect(
      ctl(svc({ updatePhoto: vi.fn().mockReturnValue({ id: 7 }) } as Partial<JourneyService>)).updatePhoto(user, '7', {
        caption: 'x',
      }),
    ).toEqual({ id: 7 });
  });
  ...
  it('DELETE photo removes the storage object when a path exists', async () => {
    expect(
      await ctl(
        svc({ deletePhoto: vi.fn().mockReturnValue({ id: 7, file_path: 'journey/a.jpg' }) } as Partial<JourneyService>),
      ).deletePhoto(user, '7'),
    ).toEqual({ success: true });
    expect(storageStub.delete).toHaveBeenCalledWith('journey', 'a.jpg');
    ...
    expect(
      await ctl(
        svc({ deletePhoto: vi.fn().mockReturnValue({ id: 8, file_path: 'journey/b.jpg' }) } as Partial<JourneyService>),
      ).deletePhoto(user, '8'),
    ).toEqual({ success: true });
    ...
    expect(
      await ctl(
        svc({
          deletePhoto: vi.fn().mockReturnValue({ id: 9, file_path: 'elsewhere/x.jpg' }),
        } as Partial<JourneyService>),
      ).deletePhoto(user, '9'),
    ).toEqual({ success: true });
  ...
  it('gallery provider-photos: batch (with passphrase), single 400/403, success', () => {
    ...
    expect(thrown(() => ctl(svc()).galleryProviderPhotos(user, '9', { provider: 'immich' }))).toEqual({
      status: 400,
      body: { error: 'provider and asset_id required' },
    });
    expect(
  ...
  it('DELETE gallery photo: 404, then removes the object when present', async () => {
    expect(
      await thrownAsync(() =>
        ctl(svc({ deleteGalleryPhoto: vi.fn().mockReturnValue(null) } as Partial<JourneyService>)).deleteGalleryPhoto(
          user,
          '7',
        ),
      ),
  ...
  it('DELETE gallery video also reclaims its uploaded poster (spec fix #2 extension)', async () => {
    ...
    await ctl(
      svc({
        deleteGalleryPhoto: vi
          .fn()
          .mockReturnValue({ id: 10, file_path: 'journey/clip.mp4', thumbnail_path: 'journey/poster.jpg' }),
      } as Partial<JourneyService>),
  ...
  it('PATCH /:id returns the updated journey on success', () => {
    expect(
      ctl(svc({ updateJourney: vi.fn().mockReturnValue({ id: 9 }) } as Partial<JourneyService>)).update(user, '9', {
        title: 'x',
      }),
    ).toEqual({ id: 9 });
  });
  ...
  it('cover upload: 400 without file, 404 when the journey is gone, else commits + returns the journey', async () => {
    expect(await thrownAsync(() => ctl(svc()).cover(user, '9', undefined))).toEqual({
      status: 400,
    ...
    expect(
      await ctl(svc({ updateJourney } as Partial<JourneyService>)).cover(user, '9', {
        filename: 'c.jpg',
      } as Express.Multer.File),
  ...
  it('DELETE /:id and trips/contributors success paths', () => {
    expect(
      ctl(svc({ deleteJourney: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).remove(user, '9'),
    ).toEqual({ success: true });
    expect(
      ctl(svc({ removeTripFromJourney: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).removeTrip(
        user,
        '9',
        '2',
      ),
    ...
    expect(
      ctl(svc({ updateContributorRole: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).updateContributor(
        user,
        '9',
        '2',
        { role: 'editor' },
    ...
    expect(
      ctl(svc({ removeContributor: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).removeContributor(
        user,
        '9',
        '2',
      ),
  ...
  it('createEntry returns the entry when the journey exists', () => {
    expect(
      ctl(svc({ createEntry: vi.fn().mockReturnValue({ id: 4 }) } as Partial<JourneyService>)).createEntry(user, '9', {
        entry_date: '2026-01-01',
      }),
    ).toEqual({ id: 4 });
  });
  ...
  it('reorderEntries succeeds for a numeric array', () => {
    expect(
      ctl(svc({ reorderEntries: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).reorderEntries(user, '9', {
        orderedIds: [3, 1, 2],
      }),
    ).toEqual({ success: true });
  });
  ...
  it('deleteShareLink returns success when removed', () => {
    expect(
      ctl(svc({ deleteJourneyShareLink: vi.fn().mockReturnValue(true) } as Partial<JourneyService>)).deleteShareLink(
        user,
        '9',
      ),
    ).toEqual({ success: true });
  ...
  it('book: 404 vs a journey with no book yet, and the save status codes', () => {
    ...
    expect(
      ctl(svc(), { getBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(true) }).getBook(user, '9'),
    ).toEqual({ book: null });
    expect(
      thrown(() =>
        ctl(svc(), { getBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(false) }).getBook(
          user,
          '9',
        ),
      ),
    ...
    expect(
      ctl(svc(), { getBook: vi.fn().mockReturnValue({ id: 3, version: 4 }), canOpen: vi.fn() }).getBook(user, '9'),
    ).toEqual({ book: { id: 3, version: 4 } });
    ...
    expect(
      thrown(() =>
        ctl(svc(), { saveBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(false) }).saveBook(
          user,
          '9',
          { document: {} } as never,
        ),
    ...
    expect(
      thrown(() =>
        ctl(svc(), { saveBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(true) }).saveBook(
          user,
          '9',
          { document: {} } as never,
        ),
    ...
    expect(
      ctl(svc(), { saveBook, broadcastSaved }).saveBook(
        user,
        '9',
        { title: 'T', document: { v: 1 }, baseVersion: 4 } as never,
        'sock-7',
      ),
    ).toEqual({ id: 3, version: 5 });
  ...
  it('book: a conflict is a 409 that carries the other version', () => {
    ...
    expect(
      thrown(() =>
        ctl(svc(), { saveBook, broadcastSaved }).saveBook(user, '9', { document: {}, baseVersion: 4 } as never),
      ),
    ).toEqual({
      status: 409,
  ...
  it('book: delete answers 204, 403 for a viewer, or 404 out of reach', () => {
    ...
    expect(
      thrown(() =>
        ctl(svc(), { deleteBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(false) }).deleteBook(
          user,
          '9',
        ),
      ),
    ...
    expect(
      thrown(() =>
        ctl(svc(), { deleteBook: vi.fn().mockReturnValue(null), canOpen: vi.fn().mockReturnValue(true) }).deleteBook(
          user,
          '9',
        ),
      ),
...
describe('JourneyPublicController', () => {
  it('is addon-gated like the authenticated surface', () => {
    ...
    expect(Reflect.getMetadata(REQUIRE_ADDON, JourneyPublicController)).toEqual({
      addonId: ADDON_IDS.JOURNEY,
      label: 'Journey',
    });
  ...
  it('legacy photo proxy: 404 invalid token, immich path streams', async () => {
    expect(
      await thrownAsync(() =>
        new JourneyPublicController(
          ...
          storageStub,
        ).legacyPhoto('tok', 'immich', 'a1', 'thumbnail', {} as Response),
      ),
    ).toEqual({ status: 404, body: { error: 'Not found' } });
    ...
    expect(streamProviderAsset).toHaveBeenCalledWith(
      {},
      'immich',
      { userId: 5, ownerId: 5, assetId: 'a1' },
      'original',
    );
  });
  ...
  it('legacy photo proxy: synology streams, and a failure becomes a 404 json', async () => {
    ...
    await new JourneyPublicController(s, storageStub).legacyPhoto(
      'tok',
      'synologyphotos',
      'a1',
      'thumbnail',
    ...
    const failing = svc({
      ...
      streamProviderAsset: vi.fn().mockRejectedValue(new Error('no synology')),
    } as Partial<JourneyService>);
  ...
  it('legacy photo proxy: ignores the path ownerId, taking the owner off the token', async () => {
    ...
    const streamProviderAsset = vi.fn().mockResolvedValue(undefined);
    const s = svc({
      validateShareTokenForAsset: vi.fn().mockReturnValue({ ownerId: 3 }),
      streamProviderAsset,
    } as Partial<JourneyService>);
    await new JourneyPublicController(s, storageStub).legacyPhoto('tok', 'immich', 'a1', 'original', {} as Response);
    expect(streamProviderAsset).toHaveBeenCalledWith(
  ...
  it('legacy photo proxy: local provider 404s when the object does not exist', async () => {
    ...
    expect(
      await thrownAsync(() =>
        new JourneyPublicController(s, storageStub).legacyPhoto(
          'tok',
          'local',
          'gone.jpg',
          'thumbnail',
  ...
  it('legacy photo proxy: a bare traversal asset id reads as a miss, not a crash', async () => {
    ...
    expect(
      await thrownAsync(() =>
        new JourneyPublicController(s, storageStub).legacyPhoto('tok', 'local', '..', 'original', {} as Response),
      ),
    ).toEqual({ status: 404, body: { error: 'Not found' } });
    expect(storageSendToResponse).not.toHaveBeenCalled();
  });
