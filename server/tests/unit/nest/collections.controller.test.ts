...
describe('CollectionsController', () => {
  ...
  it('lists / creates / reads / deletes lists (pass-throughs)', () => {
    ...
    const c = new CollectionsController(svc, new RuntimeEnvService());
  ...
  describe('reorder', () => {
    ...
    it('reorders a valid array', () => {
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).reorder(user, { orderedIds: [3, 1, 2] } as never)).toEqual({ success: true });
  ...
  describe('places', () => {
    ...
    it('deleteMany deletes a valid list', () => {
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).deleteMany(user, { ids: [1, 2] } as never, 'sid')).toEqual({ deleted: [1, 2] });
  ...
  describe('membership lookup', () => {
    it('parses lat/lng when present', () => {
      const svc = makeService();
      new CollectionsController(svc, new RuntimeEnvService()).membership(user, 'gpid', undefined, 'Rome', '41.9', '12.5');
    ...
    it('leaves lat/lng undefined when absent', () => {
      const svc = makeService();
      new CollectionsController(svc, new RuntimeEnvService()).membership(user);
  ...
  describe('invites (owner-gated)', () => {
    it('403 when a non-owner invites', () => {
      ...
      expect(thrown(() => new CollectionsController(svc, new RuntimeEnvService()).invite(user, { collection_id: 3, user_id: 2 } as never)))
    ...
    it('invites successfully as owner', () => {
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).invite(user, { collection_id: 3, user_id: 2 } as never)).toEqual({ success: true });
    });
    it('accept surfaces an error, else succeeds', () => {
      const bad = makeService({ acceptInvite: vi.fn().mockReturnValue({ error: 'Gone', status: 404 }) });
      expect(thrown(() => new CollectionsController(bad, new RuntimeEnvService()).acceptInvite(user, { collection_id: 3 } as never, 'sid')))
        .toEqual({ status: 404, body: { error: 'Gone' } });
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).acceptInvite(user, { collection_id: 3 } as never, 'sid')).toEqual({ success: true });
    ...
    it('cancel is owner-gated', () => {
      ...
      expect(thrown(() => new CollectionsController(notOwner, new RuntimeEnvService()).cancelInvite(user, { collection_id: 3, user_id: 2 } as never)))
        .toEqual({ status: 403, body: { error: 'Only the owner can cancel invites' } });
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).cancelInvite(user, { collection_id: 3, user_id: 2 } as never)).toEqual({ success: true });
    });
  });
  ...
  describe('members', () => {
    it('removeMember is owner-gated', () => {
      const notOwner = makeService({ isOwner: vi.fn().mockReturnValue(false) });
      expect(thrown(() => new CollectionsController(notOwner, new RuntimeEnvService()).removeMember(user, { collection_id: 3, user_id: 2 } as never)))
        .toEqual({ status: 403, body: { error: 'Only the owner can remove members' } });
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).removeMember(user, { collection_id: 3, user_id: 2 } as never)).toEqual({ success: true });
      expect(svc.removeMember).toHaveBeenCalledWith(1, 3, 2);
    });
    it('availableUsers is owner-gated', () => {
      const notOwner = makeService({ isOwner: vi.fn().mockReturnValue(false) });
      expect(thrown(() => new CollectionsController(notOwner, new RuntimeEnvService()).availableUsers(user, '3')))
        .toEqual({ status: 403, body: { error: 'Only the owner can manage members' } });
      expect(new CollectionsController(makeService(), new RuntimeEnvService()).availableUsers(user, '3')).toEqual({ users: [{ id: 2 }] });
    });
  });
  ...
  describe('uploadCover', () => {
    ...
    it('403 in demo mode for a demo user', () => {
      ...
      expect(thrown(() => new CollectionsController(makeService(), new RuntimeEnvService()).uploadCover(demo, '3', file)))
    ...
    it('400 when no file was uploaded', () => {
      expect(thrown(() => new CollectionsController(makeService(), new RuntimeEnvService()).uploadCover(user, '3', undefined)))
        .toEqual({ status: 400, body: { error: 'No image uploaded' } });
    });
    it('stores the cover and forwards the socket id', () => {
      const svc = makeService();
      expect(new CollectionsController(svc, new RuntimeEnvService()).uploadCover(user, '3', file, 'sid')).toEqual({ id: 3 });
  ...
  describe('uploadPlaceImage', () => {
    ...
    it('403 in demo mode for a demo user', () => {
      process.env.DEMO_MODE = 'true';
      const demo = { ...user, email: 'demo@trek.app' } as User;
      expect(thrown(() => new CollectionsController(makeService(), new RuntimeEnvService()).uploadPlaceImage(demo, '9', file)))
        .toEqual({ status: 403, body: { error: 'Uploads are disabled in demo mode. Self-host TREK for full functionality.' } });
    });
    it('400 when no file was uploaded', () => {
      expect(thrown(() => new CollectionsController(makeService(), new RuntimeEnvService()).uploadPlaceImage(user, '9', undefined)))
        .toEqual({ status: 400, body: { error: 'No image uploaded' } });
    });
    it('stores the place image via /uploads/places and forwards the socket id', () => {
      ...
      expect(new CollectionsController(svc, new RuntimeEnvService()).uploadPlaceImage(user, '9', file, 'sid')).toEqual({ id: 9 });
