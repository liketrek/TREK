...
describe('TripMembersController', () => {
  describe('members', () => {
    it('GET 404 without access, else owner+members+current_user_id', () => {
      expect(thrown(() => tc(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).members(user, '9'))).toEqual({
        status: 404,
    ...
    it('POST 403 without member_manage, else adds + notifies', () => {
      expect(
        thrown(() => tc(svc({ can: vi.fn().mockReturnValue(false) })).addMember(user, '9', { identifier: 'bob@x.y' })),
    ...
    it('POST 404 without trip access', () => {
      ...
      expect(thrown(() => tc(s).addMember(user, '9', { identifier: 'bob@x.y' }))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
    ...
    it('POST maps NotFoundError to 404, ValidationError to 400, re-throws others', () => {
      ...
      expect(thrown(() => tc(nf).addMember(user, '9', { identifier: 'bob@x.y' }))).toEqual({
        status: 404,
        body: { error: 'no user' },
      });
      const ve = svc({
      ...
      expect(thrown(() => tc(ve).addMember(user, '9', { identifier: 'bob@x.y' }))).toEqual({
        status: 400,
    ...
    it('DELETE 404 without trip access', () => {
      const s = svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) });
      expect(thrown(() => tc(s).removeMember(user, '9', '2'))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
  ...
  describe('POST /:id/transfer (#973)', () => {
    ...
    it('transfers, audits and broadcasts the refreshed trip', () => {
      const transferOwnership = vi
        .fn()
        .mockReturnValue({ tripTitle: 'Roadtrip', fromEmail: 'a@x.y', toEmail: 'b@x.y' });
    ...
    it('maps NotFoundError to 404 and ValidationError to 400', () => {
      ...
      expect(thrown(() => tc(nf).transferOwnership(user, '9', { newOwnerId: 2 }, req))).toEqual({
        status: 404,
        body: { error: 'User not found' },
      });
      const ve = svc({
        transferOwnership: vi.fn().mockImplementation(() => {
          throw new ValidationError('New owner must be a trip member');
        }),
      } as Partial<TripMembersService>);
      expect(thrown(() => tc(ve).transferOwnership(user, '9', { newOwnerId: 2 }, req))).toEqual({
        status: 400,
        body: { error: 'New owner must be a trip member' },
      });
    });
  });
  ...
  describe('guests (#1362)', () => {
    ...
    it('400 without a name; else creates', () => {
      ...
      const wsGuest = svc({
        createGuest: vi.fn().mockImplementation(() => {
          throw new ValidationError('Guest name is required');
        }),
      } as Partial<TripMembersService>);
      expect(thrown(() => tc(wsGuest).createGuest(user, '9', { name: '  ' }))).toEqual({
        status: 400,
      ...
      expect(tc(s).createGuest(user, '9', { name: 'Anna' })).toEqual({
        member: { id: 7, username: 'Anna', is_guest: true },
      });
    ...
    it('rename: 404 when the guest is missing, else success', () => {
      ...
      expect(thrown(() => tc(miss).renameGuest(user, '9', '7', { name: 'Bob' }))).toEqual({
        status: 404,
    ...
    it('delete: 404 when the guest is missing, else success', () => {
      ...
      expect(thrown(() => tc(miss).deleteGuest(user, '9', '7'))).toEqual({
        status: 404,
        body: { error: 'Guest not found' },
      });
    ...
    it('maps a ValidationError from createGuest to 400', () => {
      const ve = svc({
        createGuest: vi.fn().mockImplementation(() => {
          throw new ValidationError('Guest name must be 50 characters or fewer');
        }),
      } as Partial<TripMembersService>);
      expect(thrown(() => tc(ve).createGuest(user, '9', { name: 'x'.repeat(60) }))).toEqual({
        status: 400,
        body: { error: 'Guest name must be 50 characters or fewer' },
      });
    });
  });
