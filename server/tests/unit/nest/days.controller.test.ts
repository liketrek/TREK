...
describe('DaysController (parity with the legacy /api/trips/:tripId/days route)', () => {
  ...
  describe('PUT /reorder', () => {
    it('400 when orderedIds is missing', () => {
      expect(thrown(() => new DaysController(daysSvc()).reorder(user, '5', rawReorderBody({})))).toEqual({
        status: 400,
    ...
    it('400 when orderedIds is not an array', () => {
      expect(
        thrown(() => new DaysController(daysSvc()).reorder(user, '5', rawReorderBody({ orderedIds: 'nope' }))),
