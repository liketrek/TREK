...
Every desktop component with a phone twin (`components/Admin/<X>Panel.tsx` ↔ `mobile/screens/admin/M<X>Panel.tsx`, and the other `M*` mirrors) is a deliberate copy already ~40% duplicated. CI's SonarCloud gate allows **≤ 3% duplicated lines on the PR's new code**, so adding the same lines to both shells fails the PR by itself. Put logic in one shared hook/module under `components/Admin/` (`useInstanceSettings`, `useRangeBypass` are the pattern) and keep only markup in each shell. Full gate rules in the root `CLAUDE.md`.
