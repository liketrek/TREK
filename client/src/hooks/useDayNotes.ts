...
import type { MergedItem, DayNotesMap, DayNote, DayNoteColor } from '../types'
  ...
  const saveNote = async (dayId: number) => {
    ...
    try {
      if (ui.mode === 'add') {
        ...
        await tripStore.updateDayNote(tripId, dayId, ui.noteId!, { text: ui.text.trim(), time: ui.time || null, icon: ui.icon || 'FileText', color: ui.color ?? null })
      }
      cancelNote(dayId)
