...
export default function DashboardPage(): React.ReactElement {
  ...
  const sidebarVisible =
    (isMobile || dashCfg.desktop.sidebar) &&
...
function CurrencyTool(): React.ReactElement {
  ...
  const [rateMeta, setRateMeta] = useState<{ effectiveDate: string | null; stale: boolean }>({
    effectiveDate: null,
    stale: false,
  });

  const fetchRate = React.useCallback(() => {
    fetch(`/api/exchange-rates?base=${encodeURIComponent(from)}`)
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error('rates unavailable'))))
      .then((d: { rates?: Record<string, number>; effective_date?: string | null; stale?: boolean }) => {
        if (!d.rates) {
          setRates(null);
          return;
        }
        setRates(d.rates);
        setRateMeta({ effectiveDate: d.effective_date ?? null, stale: Boolean(d.stale) });
      })
      .catch(() => setRates(null));
  }, [from]);
  ...
  return (
    <div className="tool">
      ...
      <div className="fx-rate">
        ...
        {rateMeta.effectiveDate && <span>{rateMeta.effectiveDate}</span>}
        {rateMeta.stale && <span style={{ color: '#dc2626', fontWeight: 700 }}>{t('dashboard.fx.staleSnapshot')}</span>}
      </div>
    </div>
  );
}
