...
interface PartialLocalDateTime {
  date: string | null;
  time: string | null;
}
...
function splitLocalDateTime(value: string | null | undefined): PartialLocalDateTime {
  if (!value) return { date: null, time: null };
  if (value.includes('T')) {
    const [datePart, timePart] = value.split('T');
    return { date: normalizeDate(datePart), time: normalizeTime(timePart) };
  }
  const date = normalizeDate(value);
  return date ? { date, time: null } : { date: null, time: normalizeTime(value) };
}
...
function sameWallClock(a: WallClockParts | null, b: WallClockParts): boolean {
  return (
    !!a &&
    a.year === b.year &&
    a.month === b.month &&
...
export function zonedLocalDateTimeToEpoch(date: string, time: string, timeZone: string): number | null {
  ...
  // Sampling both sides of the target captures both offsets around a DST fold.
  // A DST gap produces no round-tripping candidate and is rejected below.
...
function resolvedLocalDateTime(reservation: Reservation, daysById: Map<number, string>): LocalDateTime | null {
  ...
  if (!date) return null;
  return { date, time: normalizeTime(endpoint?.local_time) ?? stored.time };
}

/**
 * Resolve the fixed first dated, non-cancelled transport for a trip. The first
 * transport is selected before precision checks, so an incomplete departure
 * cannot silently turn a later leg into the trip's departure transport.
 */
export function getDepartureTransport(
  ...
  const candidates = reservations
    ...
    .filter((candidate): candidate is { reservation: Reservation; local: LocalDateTime } => candidate.local !== null)
    .sort((a, b) => {
      const aKey = `${a.local.date}T${a.local.time ?? '00:00'}`;
      const bKey = `${b.local.date}T${b.local.time ?? '00:00'}`;
      return aKey.localeCompare(bKey) || a.reservation.id - b.reservation.id;
    });
