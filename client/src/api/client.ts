import {
  ...
  type TripCreateGuestRequest,
  type TripCreateRequest,
  type TripRenameGuestRequest,
...
export const tripsApi = {
  ...
  searchCoverImages: (query: string) =>
    apiClient.get('/trips/cover-images/search', { params: { query } }).then((r) => r.data),
  ...
  transferOwnership: (id: number | string, newOwnerId: number) =>
    apiClient.post(`/trips/${id}/transfer`, { newOwnerId } satisfies TripTransferOwnershipRequest).then((r) => r.data),
  createGuest: (id: number | string, name: string) =>
    apiClient.post(`/trips/${id}/guests`, { name } satisfies TripCreateGuestRequest).then((r) => r.data),
  renameGuest: (id: number | string, userId: number, name: string) =>
...
export const placesApi = {
  ...
  bulkUpdate: (tripId: number | string, ids: number[], data: Omit<PlaceBulkUpdateRequest, 'ids'>) =>
    apiClient
      .post(`/trips/${tripId}/places/bulk-update`, { ids, ...data } satisfies PlaceBulkUpdateRequest)
      .then((r) => r.data),
};
...
export const packingApi = {
  ...
  reorder: (tripId: number | string, orderedIds: number[]) =>
    apiClient
      ...
      .then((r) => r.data),
  setSharing: (tripId: number | string, id: number, data: PackingSetSharingRequest) =>
    apiClient.put(`/trips/${tripId}/packing/${id}/sharing`, data).then((r) => r.data),
  clone: (tripId: number | string, id: number) =>
    apiClient.post(`/trips/${tripId}/packing/${id}/clone`).then((r) => r.data),
  addContributor: (tripId: number | string, id: number) =>
    apiClient.post(`/trips/${tripId}/packing/${id}/contributors`).then((r) => r.data),
  removeContributor: (tripId: number | string, id: number, userId: number) =>
    apiClient.delete(`/trips/${tripId}/packing/${id}/contributors/${userId}`).then((r) => r.data),
  ...
  applyTemplate: (tripId: number | string, templateId: number, visibility: 'common' | 'personal' = 'common') =>
    apiClient
      .post(`/trips/${tripId}/packing/apply-template/${templateId}`, {
        visibility,
      } satisfies PackingApplyTemplateRequest)
      .then((r) => r.data),
...
export const adminApi = {
  ...
  pluginBrowse: (refresh?: boolean) =>
    apiClient.get('/admin/plugins/registry', { params: refresh ? { refresh: 1 } : undefined }).then((r) => r.data),
  ...
  pluginUninstall: (id: string, deleteData: boolean) =>
    apiClient.post(`/admin/plugins/${id}/uninstall`, { deleteData }).then((r) => r.data),
  pluginRescan: () => apiClient.post('/admin/plugins/rescan').then((r) => r.data),
  pluginUpload: (file: File) => {
    const fd = new FormData();
    fd.append('file', file);
    return postMultipart('/admin/plugins/upload', fd);
  },
  ...
  createInvite: (data: { max_uses: number; expires_in_days?: number; trip_id?: number | null }) =>
...
export const pluginsApi = {
  ...
  placeDetails: (placeId: number) =>
    apiClient.get(`/place-details/${placeId}`).then(
      (r) =>
        r.data as {
          providers: Array<{ pluginId: string; items: Array<{ label: string; value?: string; url?: string }> }>;
        }
    ),
  // Validation/warning contributions from warningProvider plugins (#1429). Fail-safe.
  tripWarnings: (tripId: number) =>
    apiClient.get(`/trip-warnings/${tripId}`).then(
      (r) =>
        r.data as {
          warnings: Array<{
            pluginId: string;
  ...
  viewContributions: (
    view: 'reservations' | 'transports' | 'places' | 'day' | 'costs' | 'packing' | 'files' | 'todos',
    tripId: number | string
  ) =>
  ...
  atlasLayers: () => apiClient.get('/atlas-layers').then((r) => r.data as { layers: PluginAtlasLayer[] }),
  ...
  journalEntryRows: (entryId: number) =>
    apiClient.get(`/journal-entry-rows/${entryId}`).then(
      (r) =>
        r.data as {
          providers: Array<{ pluginId: string; items: Array<{ label: string; value?: string; url?: string }> }>;
        }
  ...
  tripCardContributions: (tripIds: Array<number | string>) =>
    apiClient
      .get(`/trip-card-contributions?tripIds=${tripIds.join(',')}`)
      .then((r) => r.data as { contributions: TripCardBadge[] }),
  ...
  myActivity: (limit = 200) =>
    apiClient.get(`/plugin-activity?limit=${limit}`).then(
      (r) =>
        r.data as {
          activity: Array<{
            ts: string;
  ...
  userSettings: (id: string) =>
    apiClient.get(`/plugin-settings/${id}`).then(
      (r) =>
        r.data as {
...
export const airtrailApi = {
  ...
  import: (tripId: number, flightIds: string[], connections?: string[][]) =>
    apiClient
      .post(
        `/trips/${tripId}/reservations/import/airtrail`,
        connections?.length ? { flightIds, connections } : { flightIds }
      )
      .then((r) => r.data),
};
...
export const exchangeRateApi = {
  ...
  apply: (
    tripId: number | string,
    currency: string,
    data: { preview_id: string; selected: Array<{ type: 'expense' | 'settlement'; id: number }> }
...
export const filesApi = {
  ...
  upload: (tripId: number | string, formData: FormData, opts?: UploadOptions) =>
    postMultipart(`/trips/${tripId}/files`, formData, opts),
...
export const reservationsApi = {
  ...
  importBookingPreview: (
    tripId: number | string,
    files: File[],
    mode: BookingImportMode = 'no-ai'
...
export const healthApi = {
  features: (): Promise<{ bookingImport: boolean; aiParsing: boolean }> =>
