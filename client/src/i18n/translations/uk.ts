const uk: Record<string, string> = {
  ...
  'journey.editor.uploadPartialFailed': '{failed} з {total} фото не вдалося завантажити — збережіть ще раз, щоб повторити',
  ...
  'settings.oauth.modal.machineClientHint': 'Використовуйте надання client_credentials — URI перенаправлення не потрібні. Токен видається безпосередньо через client_id + client_secret і діє від вашого імені в межах вибраних областей.',
  'settings.oauth.modal.machineClientUsage': 'Отримати токен: POST /oauth/token з grant_type=client_credentials, client_id і client_secret. Без браузера, без токена оновлення.',
