...
type MemoriesState = ReturnType<typeof useMemoriesPanel>
...
function PhotoPickerView(S: MemoriesState) {
  ...
  return (
    ...
    {showConfirmShare && <ConfirmSharePopup {...S} />}
  </>
  )
}
...
function MainGallery(S: MemoriesState) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', ...font }}>
      ...
      {/* Lightbox */}
      {lightboxId && lightboxUserId && <MemoriesLightbox {...S} />}
    </div>
  )
}
...
export default function MemoriesPanel(props: MemoriesPanelProps) {
  ...
  if (S.loading) return <MemoriesLoading {...S} />
  if (!S.connected && S.allVisible.length === 0) return <MemoriesNotConnected {...S} />
  if (S.showAlbumPicker) return <AlbumPickerView {...S} />
  if (S.showPicker) return <PhotoPickerView {...S} />
  return <MainGallery {...S} />
}
