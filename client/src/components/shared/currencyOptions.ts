...
export function buildCurrencyOptions({
  ...
  // A saved currency may outlive the provider's current supported set. Keep it
  // selectable under Other so opening an editor never silently clears it.
  if (current && !availableSet.has(current) && !other.includes(current)) other.push(current);
  other.sort();

  if (common.length === 0) {
    return [...specialOptions, ...other.map((code) => currencyOption(code, locale))];
  }
