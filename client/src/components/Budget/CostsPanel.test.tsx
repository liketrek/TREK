...
function setViewportWidth(width: number) {
  ...
  Object.defineProperty(window, 'matchMedia', {
    ...
    value: vi.fn().mockImplementation((query: string) => ({
      matches: query === '(max-width: 1023px)' ? width <= 1023 : false,
      media: query,
      onchange: null,
      addListener: vi.fn(),
...
describe('CostsPanel — settlements in the ledger', () => {
  it('opens Trip exchange rates from the mobile total-spend card', async () => {
    const restoreViewport = setViewportWidth(390);
    try {
      server.use(
        http.get('/api/trips/1/budget', () => HttpResponse.json({ items: [] })),
        http.get('/api/trips/1/budget/settlement', () =>
          HttpResponse.json({ balances: [], flows: [], settlements: [] })
        )
      );
      ...
      await user.click(screen.getByRole('button', { name: 'Exchange rates' }));

      expect(await screen.findByText('Trip exchange rates')).toBeInTheDocument();
    } finally {
      restoreViewport();
    }
  });

  it('hides the mobile exchange-rate action without budget_edit permission', async () => {
    const restoreViewport = setViewportWidth(390);
    try {
      ...
      await screen.findByText('Total trip spend');
  ...
  it('localizes the trip exchange-rate dialog and preview status', async () => {
    seedStore(useSettingsStore, {
      settings: { ...useSettingsStore.getState().settings, language: 'de', default_currency: 'EUR' },
    });
    server.use(
      http.get('/api/trips/1/budget', () => HttpResponse.json({ items: [] })),
      http.get('/api/trips/1/budget/settlement', () => HttpResponse.json({ balances: [], flows: [], settlements: [] })),
      http.post('/api/trips/1/exchange-rates/:currency/preview', () =>
        HttpResponse.json({
          preview_id: 'localized-preview',
          rows: [
            {
              type: 'expense',
              id: 42,
    ...
    await user.click(await screen.findByRole('button', { name: 'Wechselkurse' }));
    expect(await screen.findByText('Wechselkurse der Reise')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Optionale Notiz')).toBeInTheDocument();

    await user.click(screen.getByLabelText('Bestehende Einträge aktualisieren (Vorschau erforderlich)'));
    await user.type(screen.getByPlaceholderText('0'), '0.9');
    await user.click(screen.getByRole('button', { name: 'Änderungen anzeigen' }));

    expect(await screen.findByText(/Ausgabe #42 · globaler Kurs/)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Auf 1 Einträge anwenden' })).toBeInTheDocument();
  });
  ...
  it('shows a stale suggestion and saves its value as explicit input', async () => {
    ...
    expect((screen.getByPlaceholderText('Required') as HTMLInputElement).value).toBe('0.8');
  ...
  it('requires a manual displayed rate when no snapshot exists and stores its inverse', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Required'), '0.8');
    expect(screen.getByText('Manual rate')).toBeInTheDocument();
    expect(addButtons[addButtons.length - 1]).not.toBeDisabled();
    await user.click(addButtons[addButtons.length - 1]);
    await waitFor(() => expect(posted).toMatchObject({ currency: 'USD', exchange_rate: 1.25 }));
    expect(posted).not.toHaveProperty('quote_id');
  });
