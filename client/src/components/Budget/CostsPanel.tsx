...
import { SPLIT_COLORS, SYMBOLS, currenciesWith } from './BudgetPanel.constants';
import type { TripMember } from './BudgetPanelMemberChips';
import { displayRateToStored, payersBalanced, rebalancePayers, storedRateToDisplay } from './CostsPanel.helpers';
...
export default function CostsPanel({ tripId, tripMembers = [] }: CostsPanelProps) {
  ...
  const openRateSettings = useCallback(() => setRateSettingsOpen(true), []);
  ...
  const frozenToDisplay = useCallback(
    ...
    >
          ...
          >
            ...
            {canEdit && (
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                >
                  <RotateCcw size={15} /> {t('costs.exchangeRates.manage')}
                </button>
                <button
                  onClick={settleAll}
      ...
      <style>{`
        ...
        @media (max-width: 420px) {
          .costs-root .costs-mobile-exchange-rates {
            width: 44px;
            padding: 0 !important;
          }
          .costs-root .costs-mobile-exchange-rates-label { display: none; }
        }
      `}</style>
    </div>
  );
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            ...
            {canEdit && (
              <button
                type="button"
                aria-label={t('costs.exchangeRates.manage')}
                className="costs-mobile-exchange-rates"
                onClick={openRateSettings}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
              ...
              >
                <RotateCcw size={16} />
                <span className="costs-mobile-exchange-rates-label">{t('costs.exchangeRates.manage')}</span>
              </button>
            )}
          </div>
          <div
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'center' }}>
          <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
            <div
              className="text-content"
              style={{ fontSize: 'calc(18px * var(--fs-scale-subtitle, 1))', fontWeight: 600 }}
            >
              {fmt(baseTotal(e))}
            </div>
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'center' }}>
          <div
            className="text-content"
            style={{ fontSize: 'calc(18px * var(--fs-scale-subtitle, 1))', fontWeight: 600, whiteSpace: 'nowrap' }}
          >
...
function TripRateSettings({
  ...
  const toast = useToast();
  const [rates, setRates] = useState<TripRateRow[]>([]);
  const defaultCurrency = currenciesWith('').find((c) => c !== tripCurrency) || 'USD';
  const [currency, setCurrency] = useState(defaultCurrency);
  const [value, setValue] = useState('');
  const [note, setNote] = useState('');
  const [updateExisting, setUpdateExisting] = useState(false);
  const [preview, setPreview] = useState<{ preview_id: string; rows: RatePreviewRow[] } | null>(null);
  ...
  const load = useCallback(() => {
    exchangeRateApi
      .listTrip(tripId)
      .then((result: { rates: TripRateRow[] }) => setRates(result.rates || []))
      .catch(() => {});
  }, [tripId]);
  useEffect(load, [load]);

  const storedRate = displayRateToStored(Number(value));
  const valid = storedRate !== null && currency !== tripCurrency;
  const save = async () => {
    if (!valid) return;
    setSaving(true);
    try {
      if (!updateExisting) {
        await exchangeRateApi.setTrip(tripId, currency, { exchange_rate: storedRate!, note: note || null });
        setValue('');
        load();
        onApplied();
      } else {
        const result = (await exchangeRateApi.preview(tripId, currency, {
          exchange_rate: storedRate!,
          note: note || null,
        })) as { preview_id: string; rows: RatePreviewRow[] };
        setPreview(result);
  ...
  const apply = async () => {
    ...
    setSaving(true);
    try {
      const picked = preview.rows
        .filter((row) => selected.has(`${row.type}:${row.id}`))
        .map((row) => ({ type: row.type, id: row.id }));
      await exchangeRateApi.apply(tripId, currency, { preview_id: preview.preview_id, selected: picked });
      setPreview(null);
      setValue('');
      load();
      onApplied();
      ...
      toast.error(
        error?.response?.status === 409
          ? t('costs.exchangeRates.toast.previewOutdated')
          : t('costs.exchangeRates.toast.applyError')
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button
            ...
            style={{ padding: '8px 16px', borderRadius: 10, background: 'none' }}
          >
            {t('common.close')}
          </button>
          <button
            onClick={preview ? apply : save}
            disabled={!valid || saving}
            className="bg-[var(--text-primary)] text-[var(--bg-primary)]"
            style={{ padding: '8px 16px', borderRadius: 10, border: 0, opacity: !valid || saving ? 0.5 : 1 }}
          >
            {preview
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <p className="text-content-muted" style={{ margin: 0, fontSize: 13 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: 10 }}>
          ...
          >
            ...
            <NumericInput
              ...
              placeholder="0"
              className="text-content"
              style={{ flex: 1, border: 0, background: 'none', outline: 'none', fontWeight: 600 }}
            />
            <span className="text-content-muted" style={{ fontSize: 12 }}>
              {tripCurrency}
            </span>
        ...
        />
        <label className="text-content" style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: 13 }}>
          <input
            type="checkbox"
          ...
          />{' '}
          {t('costs.exchangeRates.updateExisting')}
        </label>
        {preview && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {preview.rows.length === 0 && (
              <div className="text-content-faint">{t('costs.exchangeRates.noMatchingItems')}</div>
            )}
              ...
              return (
                ...
                >
                  ...
                  <span className="text-content">
                    {t(`costs.exchangeRates.item.${row.type}`)} #{row.id} · {exchangeRateSourceLabel(t, row.source)}
                  </span>
                  <span className="text-content-muted">
                    {row.old_trip_value.toPrecision(8)} → {row.new_trip_value.toPrecision(8)} (
                    {row.trip_value_delta >= 0 ? '+' : ''}
                    {row.trip_value_delta.toPrecision(6)} {tripCurrency})
                  </span>
                </label>
              );
            })}
        ...
        <div>
          <div className="text-content" style={{ fontWeight: 600, fontSize: 13, marginBottom: 7 }}>
            {t('costs.exchangeRates.savedDefaults')}
          </div>
          {rates.length === 0 ? (
            <div className="text-content-faint" style={{ fontSize: 12 }}>
              {t('costs.exchangeRates.noTripRates')}
            </div>
          ) : (
            rates.map((row) => (
              <div
              ...
              >
                <span className="text-content">
                  1 {row.currency} = {displayRate(row.exchange_rate)} {tripCurrency}
                </span>
                <button
                  onClick={() =>
                    exchangeRateApi.deleteTrip(tripId, row.currency).then(() => {
                      load();
                      onApplied();
                    })
                  }
...
function useItemRate(
  ...
  useEffect(() => {
    ...
    existing?.exchange_rate_effective_date,
  ]);
  ...
  const valid = currency.toUpperCase() === tripCurrency.toUpperCase() || (Number.isFinite(numeric) && numeric > 0);
  const write =
...
function SettlementModal({
  ...
  const [saving, setSaving] = useState(false);
  const rate = useItemRate(tripId, cur, tripCurrency, editing);

  const amt = parseFloat(amount) || 0;
  const valid = amt > 0 && fromId !== toId && rate.valid && !rate.loading;
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {cur !== tripCurrency && (
          <div>
            <label className={labelCls}>{t('costs.exchangeRates.label')}</label>
            <div
              className="border border-edge bg-surface-input"
              style={{
                display: 'flex',
            ...
            <div className="text-content-faint" style={{ fontSize: 11.5, marginTop: 5 }}>
              ...
              {rate.unavailable && (
                <span style={{ color: '#dc2626' }}> · {t('costs.exchangeRates.manualRequired')}</span>
              )}
            </div>
          </div>
        )}
...
export function ExpenseModal({
  ...
  const [currency, setCurrency] = useState((editing?.currency || tripCurrency).toUpperCase());
  const rate = useItemRate(tripId, currency, tripCurrency, editing);
  ...
  const ticketValid =
    ...
    ticketItems.every(
      (item) => item.name.trim().length > 0 && (parseFloat(item.price) || 0) > 0 && item.participants.size > 0
    );
        ...
        {currency !== tripCurrency && (
          <div>
            ...
            <div className="text-content-faint" style={{ fontSize: 11.5, marginTop: 5 }}>
              ...
              {totalNum > 0 && rate.valid && (
                <span>
                  {' '}
                  · {formatMoney(totalNum, currency, locale)} ≈{' '}
                  {formatMoney(totalNum * Number(rate.value), tripCurrency, locale)}
                </span>
              )}
            </div>
          </div>
