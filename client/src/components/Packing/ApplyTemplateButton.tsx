...
interface ApplyTemplateButtonProps {
  ...
  className?: string
  targetVisibility?: 'common' | 'personal'
}

// Dropdown-Button um ein Packing-Template auf den aktuellen Trip anzuwenden.
// Rendert nichts wenn keine Templates existieren.
export default function ApplyTemplateButton({ tripId, style, className, targetVisibility = 'common' }: ApplyTemplateButtonProps): React.ReactElement | null {
  ...
  const handleApply = async (templateId: number) => {
    setApplying(true)
    try {
      const data = await packingApi.applyTemplate(tripId, templateId, { visibility: targetVisibility })
