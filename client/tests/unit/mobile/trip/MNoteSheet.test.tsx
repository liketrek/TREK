...
describe('MNoteSheet', () => {
  ...
  it('FE-MOB-NOTESH-005: appends the new note behind the last assignment and note of the day', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('dayplan.noteSubtitle'), { target: { value: '18:00 at the gate' } })
  ...
  it('FE-MOB-NOTESH-009: prefills icon, title and detail when editing an existing note', () => {
    ...
    expect(screen.getByPlaceholderText('dayplan.noteSubtitle')).toHaveValue('09:30 at the kiosk')
  ...
  it('FE-MOB-NOTESH-015: counts the detail characters and warns near the limit', () => {
    renderSheet()
    const detail = screen.getByPlaceholderText('dayplan.noteSubtitle')
    expect(screen.getByText('0/250')).toBeInTheDocument()
    fireEvent.change(detail, { target: { value: 'a'.repeat(100) } })
    const relaxed = screen.getByText('100/250')
    expect(relaxed.className).toContain('text-m-faint')

    fireEvent.change(detail, { target: { value: 'a'.repeat(241) } })
    const warned = screen.getByText('241/250')
