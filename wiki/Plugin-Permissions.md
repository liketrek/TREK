...
| `notify:send` | Send a persisted notification (bell inbox + email/ntfy/webhook/Web Push fan-out) via `ctx.notify.send` | Host-mediated: the host owns recipient resolution, channel fan-out and per-user preferences. Recipients are **forced** to the acting user (`scope:'user'`, `targetId` = the acting user) or a trip they belong to (`scope:'trip'`); `scope:'admin'` is refused. The plugin supplies only a plain-text title/body (caps 200/1000) + an optional in-app `link` (must be a relative `/…` path, which keeps it open-redirect-safe). No arbitrary recipient, no impersonation. Budgeted: 100 sends/day per plugin, resetting at UTC midnight. |
...
| `hook:search-provider` | Answer place searches from an index TREK does not ship, beside its own results | Implement `SearchProvider` in `hooks` on the plugin definition — hits are drawn into the app’s search list; also exposed at `GET /api/plugin-search` and as the `search_places_via_plugins` MCP tool. A plugin that also implements the optional `suggest` is asked while the user types, so it receives what is typed into the place search from the second character on, not only the finished query, and adds up to three rows to the dropdown (`GET /api/plugin-search/suggest`). Only these results can carry a `rating`: open data has none. The Road trip search along the route calls the same hook once per kind, with `category` and `bounds` added to the request and a fixed category phrase as `query`; hits outside `bounds`, or of a different road trip kind, are dropped. |
| `hook:poi-category-provider` | Add its own place categories to the trip map's **Explore places** pill and answer them for the map area the user is viewing | Declare up to 4 chips in `capabilities.poiCategories` (each `{id, label, labels?, icon, color}`: the icon from a fixed list of lucide names, the colour `#rrggbb` only) and implement `PoiCategoryProvider` in `hooks`. When a user picks one of the chips, TREK calls `getPois({category, bounds, lang?, limit})` on exactly that plugin, **as that user**, with the viewport narrowed to at most 0.5 degrees a side, so granting this means the plugin learns which map area each user who uses its chips is looking at. Its own permission for that reason, rather than a reuse of `hook:search-provider`: that hook is asked about a search somebody ran, this one about the map area somebody is looking at. The host keeps at most 60 places inside `bounds`, caps every string and the up-to-6 `details` rows, allows only http/https websites, namespaces ids as `plugin:<id>:<ref>`, and styles every marker with the declared icon and colour rather than anything the answer carries. 8 s timeout; a slow or failing plugin costs only its own chip. Nothing is cached, because the answer may depend on the user's own settings. Without the grant a declaration still installs but its chips never appear. Exposed at `GET /api/plugin-pois` and as the `list_plugin_poi_categories` and `search_plugin_pois` MCP tools. |
