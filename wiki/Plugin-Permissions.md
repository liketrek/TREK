...
| `hook:poi-category-provider` | Register named **POI categories** (with icon + color) and answer place-search queries via the `hooks.poiCategoryProvider` provider hook | Implement `PoiCategoryProvider` in `hooks` — `getCategories(ctx)` returns `{id, name, icon?, color?}[]` once after activation (category chips shown in the UI); `search(opts, ctx)` returns `{results: PoiResult[], hasMore}` for each user query. `opts` carries `{query?, bounds?: {north, south, east, west}, limit?}` (limit clamped 1–100, default 20). Each result: `{id, categoryId, name, lat, lng, address?, description?, url?, icon?}`. The host normalizes all fields (string-coerce + cap: name ≤120, address ≤300, description ≤500), range-checks coordinates (−90..90 / −180..180), allowlists the url (http/https/mailto only), caps results at 100 per provider, and skips a failing call — never fatal. Exposed at `GET /api/plugin-poi-categories`. See the `poi-demo` example plugin. |
