...
```bash
git remote add upstream git@github.com:iiketrek/TREK.git
```
...
| `upstream` | `git@github.com:iiketrek/TREK.git`         | Main repo — pull updates from here |
...
Then open a Pull Request from your fork to `iiketrek/TREK` targeting the `dev` branch. If your PR only modifies files under `wiki/`, it is exempt from branch enforcement and may target any branch.
