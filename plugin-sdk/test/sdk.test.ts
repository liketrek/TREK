...
describe('pack + entry (publishing automation)', () => {
  ...
  it('builds a registry entry with sha256/size/commit/trek filled in', () => {
    ...
    const entry = buildEntry({
      dir: koffi, repo: 'iiketrek/TREK-plugin-koffi', tag: 'v1.0.0', zipPath: out,
  ...
  it('--merge refreshes the icon from the manifest, but never wipes one the entry already carries', () => {
    ...
    const prior = (icon: string) => {
      ...
      fs.writeFileSync(p, JSON.stringify({
        ...
        repo: 'iiketrek/TREK-plugin-koffi', type: 'widget', icon,
  ...
  it('--merge prepends the new version onto an existing entry, newest-first', () => {
    ...
    fs.writeFileSync(existingPath, JSON.stringify({
      id: 'koffi', name: 'Koffi', author: 'TREK', description: 'x', repo: 'iiketrek/TREK-plugin-koffi', type: 'widget',
...
describe('sign + keygen (author signatures, TOFU)', () => {
  ...
  it('entry --sign fills signature + authorPublicKey that verify against the artifact', () => {
    ...
    const keyPath = path.join(tmp, 'k.pem');
    const { publicKey } = generateKeypair(keyPath);
    const entry = buildEntry({ dir: koffi, repo: 'iiketrek/TREK-plugin-koffi', tag: 'v1.0.0', zipPath: out, commit: 'a'.repeat(40), signKeyPath: keyPath, now: '2026-07-04T00:00:00.000Z' });
  ...
  it('refuses to sign an update with a different key than the one already published', () => {
    ...
    const existing = buildEntry({ dir: koffi, repo: 'iiketrek/TREK-plugin-koffi', tag: 'v1.0.0', zipPath: out, commit: 'a'.repeat(40), signKeyPath: key1, now: '2026-07-04T00:00:00.000Z' });
    const existingPath = path.join(tmp, 'koffi.json');
    ...
    expect(() => buildEntry({ dir: koffi, repo: 'iiketrek/TREK-plugin-koffi', tag: 'v1.1.0', zipPath: out, commit: 'a'.repeat(40), mergePath: existingPath, signKeyPath: key2, now: '2026-07-04T00:00:00.000Z' })).toThrow(/differs from the one already published/);
  });
});
