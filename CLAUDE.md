...
- **DB** (`server/src/db/`): `better-sqlite3`, WAL, FK on. Schema, migrations and seeding are **MikroORM's** (`db/migrations/`, `db/seeders/`), applied at boot from `buildApp()`; the ORM shares the one better-sqlite3 connection rather than opening its own. Queries are still raw SQL through `DatabaseService`. `NODE_ENV=test` gives each vitest worker an isolated `:memory:` DB.
