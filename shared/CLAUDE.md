...
- **Every locale gets a real translation — an English placeholder is not acceptable.** Parity only checks that the key exists, so copying the `en` string into the other 22 locales passes CI while shipping English to those users. Write the translation in that locale's language (native phrasing, the locale's own quotation marks and punctuation, the same `{placeholders}` as `en`), and if you genuinely cannot translate one, say so in the summary instead of filling it with English.
