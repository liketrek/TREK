...
const backup: TranslationStrings = {
  ...
  'backup.target.backend': 'Speicher-Backend',
  'backup.target.backendHint': 'Backups bleiben immer auf diesem Server. Ein Backend legt zusätzlich eine zweite Kopie an anderer Stelle an — es ersetzt die lokale nie.',
  'backup.target.backend.none': 'Nur lokal',
  'backup.target.backend.local': 'Verzeichnis',
  'backup.target.backend.s3': 'S3-kompatibel',
  'backup.target.localPath': 'Zielverzeichnis',
  'backup.target.localPathHint': 'Absoluter Pfad. Beim Docker-Image ist das ein Pfad im Container, den du auf den gewünschten Zielort mappst; bei einer Quell-Installation ist es ein normaler Dateisystempfad. Muss außerhalb der TREK-Verzeichnisse data und uploads liegen.',
  ...
  'backup.target.managedByEnv': 'Dieses Ziel wird über BACKUP_TARGET_TYPE und die zugehörigen Umgebungsvariablen konfiguriert und kann hier nicht bearbeitet werden.',
