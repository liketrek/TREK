...
const admin: TranslationStrings = {
  ...
  'admin.authMethods': '登入方式',
  'admin.passwordLogin': '密碼登入',
  'admin.passwordLoginHint': '允許使用者使用電子郵件和密碼登入',
  'admin.passwordRegistration': '密碼註冊',
  'admin.passwordRegistrationHint': '允許新使用者使用電子郵件和密碼註冊',
  'admin.oidcLogin': 'SSO 登入',
  'admin.oidcLoginHint': '允許使用者透過 SSO 登入',
  'admin.oidcRegistration': 'SSO 自動建立帳戶',
  'admin.oidcRegistrationHint': '自動為首次透過 SSO 登入的使用者建立帳戶',
  'admin.envOverrideHint': '密碼登入設定由 OIDC_ONLY 環境變數控制，無法在此處變更。',
  'admin.lockoutWarning': '至少必須保留一種登入方式',
  ...
  'admin.plugins.perm.db:write:atlas': '標記操作使用者已造訪的國家與地區並管理願望清單（需要足跡附加元件）',
  'admin.plugins.perm.db:write:vacay': '在操作使用者的使用中計畫中切換休假日與公司假日（需要假期附加元件）',
  'admin.plugins.perm.db:write:journal': '在操作使用者可編輯的日誌中建立、編輯及刪除項目（需要旅程附加元件）',
  'admin.plugins.perm.db:write:collections':
    '建立及編輯收藏並將地點儲存至其中，依操作使用者的收藏角色執行（需要收藏附加元件）',
  'admin.plugins.perm.db:write:files':
    '向操作使用者可編輯的行程附加檔案（10MB 上限，被封鎖的副檔名將被拒絕）並管理其連結',
  'admin.plugins.perm.db:write:collab': '在操作使用者可編輯的行程中發佈筆記、投票及聊天訊息（需要協作附加元件）',
  ...
  'admin.plugins.perm.db:read:journal': '讀取操作使用者的旅行日誌（需要旅程附加元件）',
  'admin.plugins.perm.db:read:atlas': '讀取操作使用者已造訪的國家與地區（需要足跡附加元件）',
  'admin.plugins.perm.db:read:vacay': '讀取操作使用者的假期計畫（需要假期附加元件）',
  'admin.plugins.perm.db:read:daynotes': '讀取操作使用者可存取之行程的每日備註',
  'admin.plugins.perm.db:read:collections': '讀取操作使用者已儲存地點的收藏（需要收藏附加元件）',
  ...
  'admin.plugins.perm.hook:atlas-layer-provider': '在足跡世界地圖上標示國家（例如願望清單或旅遊警示）',
  ...
  'admin.plugins.perm.db:read:collab': '讀取操作使用者可存取之行程的筆記、投票及聊天訊息（需要協作附加元件）',
  ...
  'admin.addons.catalog.atlas.name': '足跡',
  ...
  'admin.update.dataInfo': '您的所有資料（行程、使用者、API 金鑰、上傳檔案、假期、足跡、預算）都會保留。',
  ...
  'admin.passkey.title': '通行密鑰 登入',
  'admin.passkey.cardHint': '讓使用者使用 通行密鑰（WebAuthn）登入。預設為關閉。',
  'admin.passkey.login': '啟用 通行密鑰 登入',
  'admin.passkey.loginHint': '顯示「使用 通行密鑰 登入」選項，並讓使用者在設定中註冊 通行密鑰。',
  'admin.passkey.notConfigured':
    '此部署尚未解析出任何 WebAuthn 網域。請設定下方的 APP_URL 或 Relying Party ID——在此之前通行密鑰將保持隱藏。',
  'admin.passkey.rpId': 'Relying Party ID（網域）',
  'admin.passkey.rpIdHint':
    '通行密鑰綁定的純網域，例如 trek.example.org。留空則從 APP_URL 推導。日後變更將使現有通行密鑰失效。',
  'admin.passkey.origins': '允許的來源',
  'admin.passkey.originsHint': '以逗號分隔的完整來源，例如 https://trek.example.org。留空則使用 APP_URL。',
  'admin.passkey.reset': '重設 通行密鑰',
  'admin.passkey.resetHint': '移除此使用者的所有 通行密鑰（例如裝置遺失時）。他們仍可使用密碼登入。',
  'admin.passkey.resetConfirm': '要移除 {name} 的所有 通行密鑰 嗎？',
  'admin.passkey.resetDone': '已移除 {count} 個 通行密鑰',
  ...
  'admin.group.users': '使用者',
  'admin.group.config': '設定',
  'admin.group.integration': '整合',
  'admin.group.maintenance': '維護',
