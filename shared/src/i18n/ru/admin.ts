...
const admin: TranslationStrings = {
  ...
  'admin.plugins.devLinkHint': 'Загружено из локального каталога сборки и горячо перезагружается с реальными данными — только для разработки',
  ...
  'admin.plugins.perm.db:write:packing': 'Создавать, редактировать и удалять пункты списка вещей в поездках, которые текущий пользователь может редактировать',
  ...
  'admin.plugins.perm.db:write:todos': 'Создавать, редактировать и удалять задачи в поездках, которые текущий пользователь может редактировать',
  'admin.plugins.perm.db:write:atlas': 'Отмечать страны и регионы как посещённые и управлять списком желаний текущего пользователя (требуется дополнение Atlas)',
  'admin.plugins.perm.db:write:vacay': 'Переключать дни отпуска и корпоративные праздники в активном плане текущего пользователя (требуется дополнение Vacay)',
  'admin.plugins.perm.db:write:journal': 'Создавать, редактировать и удалять записи в дневниках, которые текущий пользователь может редактировать (требуется дополнение Journey)',
  'admin.plugins.perm.db:write:collections': 'Создавать и редактировать коллекции и сохранять в них места с ролью текущего пользователя в коллекции (требуется дополнение Collections)',
  'admin.plugins.perm.db:write:files': 'Прикреплять файлы к поездкам, которые текущий пользователь может редактировать (лимит 10 МБ, заблокированные расширения отклоняются), и управлять их связями',
  'admin.plugins.perm.db:write:collab': 'Публиковать заметки, опросы и сообщения чата в поездках, которые текущий пользователь может редактировать (требуется дополнение Collab)',
  'admin.plugins.perm.db:write:members': 'Добавлять пользователей в поездки — предоставляет доступ к поездке; защищено правом управления участниками текущего пользователя',
  'admin.plugins.perm.notify:send': 'Отправлять уведомление (колокольчик + email/ntfy) текущему пользователю или его поездке — никогда произвольному получателю',
  'admin.plugins.perm.ai:invoke': 'Использовать настроенного администратором ИИ-провайдера (дополнение текста + извлечение из документов) от имени текущего пользователя — плагин никогда не хранит ключ',
  'admin.plugins.perm.oauth:client': 'Подключаться к стороннему сервису от вашего имени через OAuth, посредником которого выступает хост (хост хранит токены, плагин их не видит)',
  'admin.plugins.perm.db:read:files': 'Читать файлы поездок, доступных текущему пользователю',
  'admin.plugins.perm.db:write:reservations': 'Создавать, редактировать и удалять бронирования в поездках, которые текущий пользователь может редактировать',
  'admin.plugins.perm.db:write:accommodations': 'Создавать, редактировать и удалять размещения в поездках, которые текущий пользователь может редактировать',
  'admin.plugins.perm.db:read:journal': 'Читать дневники путешествий текущего пользователя (требуется дополнение Journey)',
  'admin.plugins.perm.db:read:atlas': 'Читать посещённые страны и регионы текущего пользователя (требуется дополнение Atlas)',
  ...
  'admin.plugins.perm.db:read:collections': 'Читать коллекции сохранённых мест текущего пользователя (требуется дополнение Collections)',
  'admin.plugins.perm.db:write:daynotes': 'Создавать, редактировать и удалять заметки дня в поездках, которые текущий пользователь может редактировать',
  'admin.plugins.perm.jobs:run': 'Запускать объявленные фоновые задачи по расписанию (без контекста пользователя — не может читать данные пользователя)',
